...
!**********************************************************************************************************************!
! subroutine MapRaw2Mech - map explicit species to mechanism species
!**********************************************************************************************************************!
subroutine MapRaw2Mech(mech)
  integer(kind=i4)  :: iz
  character(len=11) :: mech

  ! map to the RACM2+ species
  do iz=1,npts
    q(iz,iISO)    = qraw(iz,bisop)+qraw(iz,bmbo)
    q(iz,iLIM)    = qraw(iz,bmyrc)+qraw(iz,blimo)+qraw(iz,bocim)+qraw(iz,bpcym)+ &
                    qraw(iz,bocym)+qraw(iz,batrp)+qraw(iz,bgtrp)+qraw(iz,bbphe)+ &
                    qraw(iz,bafrn)+qraw(iz,bbcro) 
    q(iz,iAPI)    = qraw(iz,bsabi)+qraw(iz,bcare)+qraw(iz,bapin)+qraw(iz,bbpin)
    q(iz,iMOH)    = qraw(iz,bmeoh)
    q(iz,iACT)    = qraw(iz,bacet)
    q(iz,iETE)    = qraw(iz,bethe)
    q(iz,iOLT)    = qraw(iz,bbute)
    q(iz,iETH)    = qraw(iz,betha)
    q(iz,iEOH)    = qraw(iz,betoh)
    q(iz,iACD)    = qraw(iz,baald)
    q(iz,iHCHO)   = qraw(iz,bhcho)
    q(iz,iORA1)   = qraw(iz,bform)
    q(iz,iORA2)   = qraw(iz,bacta)
    q(iz,iNO)     = 0.0
    q(iz,iCO)     = qraw(iz,bco)
  end do

end subroutine MapRaw2Mech
