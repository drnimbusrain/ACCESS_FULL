...
module Utils
  ...
  public SolveTridiag, & 
         CleanUp, &
         ConvertPPBToMolecCC, ConvertMolecCCToPPB, &
         Convertugm3ToMolecCC, ConvertMolecCCTougm3, &
         ConvertEmissionRate, &
         ConvertFluxToStdFormat, &
         ConvertFluxToOutputFormat, &
         ConvertOneToOutputFormat, &
         RelativeHumidity, SpecificHumidity, &
         Convert_qh_to_h2o, Convert_h2o_to_qh, &
         rtog, gtor, &
         DateTime2SimTime, TimeString, &
         SolarZenithAngle
...
subroutine CleanUp()
  ...
  ! close constrained species file
  close(UCNS)
...
!**********************************************************************************************************************!
! function ConvertFluxToStdFormat - convert flux from molecules/cm2-s to
!                                      nmol/m2-s
!**********************************************************************************************************************!
function ConvertFluxToStdFormat(fmcc)
  real(kind=dp)    :: fmcc
  real(kind=dp)    :: ConvertFluxToStdFormat

  ConvertFluxToStdFormat = fmcc*1.0D+13/navo

  return
end function ConvertFluxToStdFormat

!**********************************************************************************************************************!
! function ConvertFluxToOutputFormat - based on data provided in the simulation
!                                      control file (via outppb) either
!                                      convert to ppbv-cm/s or leave as
!                                      molecules/cm2-s
!**********************************************************************************************************************!
function ConvertFluxToOutputFormat(fmcc, isp, izpt)
  ...
  ! then convert source to ppbv-cm/s, otherwise leave as molecules/cm2-s
  if (ANY(outppb == isp)) then
    ConvertFluxToOutputFormat = fmcc*1.0D+09/cair(izpt)
  else
    ConvertFluxToOutputFormat = fmcc
  end if
...
!                                     either convert to ppbv or leave as
!                                     molecules/cm3
!**********************************************************************************************************************!
function ConvertOneToOutputFormat(cmcc, isp, izpt)
  ...
  ! if species isp is contained in the array outppb, 
  ! then convert it to PPB, otherwise leave it the same (molec/cm3)
  if (ANY(outppb == isp)) then
    ConvertOneToOutputFormat = ConvertMolecCCToPPB(cmcc, izpt)
  else
    ConvertOneToOutputFormat = cmcc 
  end if
